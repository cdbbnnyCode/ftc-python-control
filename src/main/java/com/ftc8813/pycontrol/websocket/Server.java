...
public class Server
{
    ...
    private static final String TAG = "Socket Control";
    ...
    public void close()
    {
        try
        {
            ...
            {
                RobotLog.dd(TAG, "Closing server");
                server.close();
            }
            if (worker.connection != null)
            {
                ...
                {
                    RobotLog.dd(TAG, "Closing connection");
                    conn.close();
                }
            }
    ...
    {
        private final String TAG = "Socket Worker";
        ...
        {
            this.server = server;
            RobotLog.dd(TAG, "Initialized worker for port %d", port);
        ...
        public void run()
        {
            ...
            try (ServerIO server = this.server)
            {
                while (true)
                {
                    ...
                    RobotLog.dd(TAG, "Waiting for connection on port %d...", port);
                    try (SocketIO sock = server.accept())
                    {
                        connection = new WeakReference<>(sock);
                        RobotLog.dd(TAG, "Connection from %s", sock.getConnectionInfo());
                        ...
                        while (!closeConnection)
                        {
                            ...
                            {
                                RobotLog.ww(TAG, "Connection lost [could not receive command]");
                                break;
                            }
                            ...
                            else
                            {
                                ...
                                if (processor == null)
                                {
                                    RobotLog.ww(TAG, "No processor for command %d; echoing request", processor);
                                    processor = new CmdEcho();
                                }
                            }
                    ...
                    catch (SocketException e)
                    {
                        ...
                        {
                            RobotLog.ww(TAG, "Server closed: %s", e.getMessage());
                            break;
                        }
                        RobotLog.ee(TAG, e, "Server error");
                        RobotLog.ee(TAG, "Connection failed -- listening for new connections");
                    }
                }
            }
            catch (IOException e)
            {
                RobotLog.ee(TAG, e, "IO error");
            }
            finally
            {
                this.server = null;
